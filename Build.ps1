...
if(Test-Path (Join-Path $SourcePath Public) -Type Container){
    ...
    Set-ItemProperty $ReleaseManifest -name IsReadOnly -value $false
...
## Bump the version (note that this doesn't change source code, you're still responsible)
Update-Metadata -Path $ReleaseManifest -PropertyName 'ModuleVersion' -Value $Version

## Validate
Write-Host "`nVALIDATE: ScriptAnalyzer on $ReleasePath"
Invoke-ScriptAnalyzer $ReleasePath -Recurse

Write-Output (Get-Item $ReleasePath)